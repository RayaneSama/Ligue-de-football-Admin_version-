...
exports.comptegestionnaire = (req, res) => {
  ...
  db.query(
    "SELECT c.nom_utilisateur, e.nom_eq FROM compte c JOIN equipe e ON c.id_co = e.id_co_ge_eq WHERE c.id_co = ?",
    [userId],
    (err, results) => {
      if (err) {
      ...
      res.render("../views/Gestionnaire/homepageGestionnaire", {
        locals,
        userId,
        results,
      });
...
exports.compteadmin = async (req, res) => {
  ...
  db.query(
    "SELECT c.nom_utilisateur, a.id_ad FROM compte c JOIN admin a ON c.id_co = a.id_compte_ad WHERE c.id_co = ?",
    [userId],
    (err, results) => {
      if (err) {
        console.error("Erreur SQL : " + err);
