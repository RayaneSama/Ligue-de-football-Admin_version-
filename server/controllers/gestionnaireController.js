...
exports.postPlayer = async (req, res) => {
  ...
  db.query(
    ...
    [
      2,
      newjoueur.nom + " " + newjoueur.prenom,
      newjoueur.motdepasse,
      newjoueur.email,
    ],
    (err, result) => {
      if (err) {
      ...
      db.query(
        ...
        (err, resultid) => {
          ...
          db.query(
            "SELECT id_eq FROM equipe  WHERE id_co_ge_eq=?",
            [userid],
            (err, resultideq) => {
              if (err) {
                console.error("erreur avoir id  compte " + err);
                return res
                  .status(500)
                  .send("erreur sql avoir id compte du joueur");
              }
              console.log(resultideq); // id de lequipe que le gestionnaire gere
              db.query(
                "INSERT INTO joueur (nom_jo, prenom_jo, nationalite_jo, poste_jo, num_mai_jo, age_jo, taille_jo, poids_jo,id_eq_jo,id_co_jo) VALUES (?,?,?,?,?,?,?,?,?,?) ",
                [
                  newjoueur.nom,
                  newjoueur.prenom,
...
exports.viewPlayer = async (req, res) => {
  ...
  db.query(
    "SELECT id_jo,nom_jo, prenom_jo, nationalite_jo, poste_jo, num_mai_jo, age_jo, taille_jo, poids_jo,compte.email_co FROM joueur JOIN compte ON joueur.id_co_jo = compte.id_co AND id_jo=?;",
    [userId],
    (err, result) => {
      if (err) {
      ...
      res.render("../views/Gestionnaire/details", {
        locals,
        id,
        result,
        userId,
      }); // Rend la vue avec les détails du joueur
    }
  );
};
...
exports.editPlayer = async (req, res) => {
  ...
  // Exécute une requête SQL pour récupérer les détails du joueur avec l'ID spécifié
  db.query(
    "SELECT id_jo,nom_jo, prenom_jo, nationalite_jo, poste_jo, num_mai_jo, age_jo, taille_jo, poids_jo,compte.email_co,mot_de_passe FROM joueur JOIN compte ON joueur.id_co_jo = compte.id_co AND id_jo=?;",
    [playerId],
    (err, result) => {
      if (err) {
        console.error("erreur sql id joueur" + err);
      ...
      const id = result[0].id_jo;
      res.render("../views/Gestionnaire/modifier", {
        locals,
        id,
        result,
        userId,
...
exports.editpost = async (req, res) => {
  ...
  db.query(
    "UPDATE compte JOIN joueur ON compte.id_co = joueur.id_co_jo SET compte.id_type=?, compte.nom_utilisateur=?, compte.mot_de_passe=?, compte.email_co=? WHERE joueur.id_jo=?",
    [
      2,
      newjoueur.nom + " " + newjoueur.prenom,
      newjoueur.motdepasse,
    ...
    (err, result) => {
      ...
      db.query(
        ...
        (err, resultid) => {
          ...
          db.query(
            "UPDATE joueur SET nom_jo=?, prenom_jo=?, nationalite_jo=?, poste_jo=?, num_mai_jo=?, age_jo=?, taille_jo=?, poids_jo=?,id_co_jo=? WHERE id_jo=?",
            [
              newjoueur.nom,
              newjoueur.prenom,
              newjoueur.nationalite,
...
exports.supprimerJoueur = async (req, res) => {
  ...
  db.query(
    "SELECT id_co_jo FROM joueur WHERE id_jo=?",
    [req.params.id],
    (err, res) => {
      if (err) {
        ...
        return res
          .status(500)
          .send("erreur sql avoir id compte joueurs supprime");
      }
      console.log(res);
      db.query(
        "DELETE FROM joueur WHERE id_jo= ?",
        [req.params.id],
        (err, result) => {
          if (err) {
...
exports.gererJoueur = async (req, res) => {
  ...
  db.query(
    "SELECT joueur.id_jo, joueur.nom_jo, joueur.prenom_jo, joueur.nationalite_jo, joueur.poste_jo, joueur.num_mai_jo, joueur.age_jo, joueur.taille_jo, joueur.poids_jo, compte.email_co FROM joueur JOIN equipe ON joueur.id_eq_jo = equipe.id_eq JOIN compte ON joueur.id_co_jo = compte.id_co WHERE equipe.id_co_ge_eq = ?; ",
    [userId],
    (err, result) => {
      if (err) {
...
exports.search = async (req, res) => {
  ...
  db.query(
    "SELECT joueur.id_jo, joueur.nom_jo, joueur.prenom_jo FROM joueur JOIN equipe ON joueur.id_eq_jo = equipe.id_eq JOIN compte ON joueur.id_co_jo = compte.id_co WHERE (joueur.nom_jo LIKE ? OR joueur.prenom_jo LIKE ?) AND equipe.id_co_ge_eq = ?",
    [`%${recherche}%`, `%${recherche}%`, userId],
    (err, result) => {
      if (err) {
...
exports.monprofile = async (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      db.query(
        "SELECT nom_eq FROM equipe WHERE id_co_ge_eq=?",
        [userId],
        (err, results) => {
          if (err) {
          ...
          res.render("../views/Gestionnaire/profile", {
            userId,
            locals,
            result,
            results,
...
exports.monprofilepost = async (req, res) => {
  ...
  db.query(
    "SELECT mot_de_passe FROM compte WHERE id_co=?",
    [userId],
    (err, result) => {
      if (err) {
      ...
      if (mdpactuel === result[0].mot_de_passe) {
        // Update password
        db.query(
          "UPDATE compte SET mot_de_passe=? WHERE id_co=?",
          [nvmdp, userId],
          (err, result) => {
            if (err) {
